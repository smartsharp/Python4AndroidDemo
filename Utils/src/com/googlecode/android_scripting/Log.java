  ...
  public static void notify(Context context, String title, String contentTitle, String message) {
    ...
    if (Build.VERSION.SDK_INT < 16) {
      Notification.Builder builder = new Notification.Builder(context).setAutoCancel(true)
          .setContentTitle(contentTitle).setContentText(message)
          .setContentIntent(PendingIntent.getService(context, 0, null, 0))
          .setSmallIcon(iconId > 0 ? iconId : -1).setWhen(System.currentTimeMillis())
          .setOngoing(true);
      note = builder.getNotification();
    } else {
      note = new Notification.Builder(context).setAutoCancel(true).setContentTitle(contentTitle)
          .setContentText(message).setContentIntent(PendingIntent.getService(context, 0, null, 0))
          .setSmallIcon(iconId > 0 ? iconId : -1).setWhen(System.currentTimeMillis()).build();
    }
